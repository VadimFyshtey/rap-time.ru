    ...
    <div class="content-wrapper">
        <section class="content-header">
            ...
            <ol class="breadcrumb">
                ...
                <li class="active">Комментарии к новостям</li>
            </ol>
        </section>
        @if(session('status'))
            ...
            <table class="table table-hover">
                ...
                @foreach($comments as $comment)
                    <tr>
                        ...
                        <td><a href="{{ route('newsView', ['alias' => $comment->news->alias, 'id' => $comment->news->id]) }}" rel="nofollow" target="_blank">{{ $comment->news->title }}</a></td>
                        <td><a href="{{ route('adminUserEdit', ['id' => $comment->user->id]) }}" target="_blank" rel="nofollow">{{ $comment->user->name }}</a></td>
                        <td class="action-admin">
                            <a href="{{ route('adminCommentNewsDelete', ['id' => $comment->id]) }}" onclick="return confirm('Вы действительно хотите удалить этот комментарий?')"><i class="glyphicon glyphicon-remove"></i></a>
                        </td>
                    </tr>
                @endforeach
                </tbody>
